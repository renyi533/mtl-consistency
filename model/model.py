...
def task_coattention(original_tower_inputs, args, mode):
    with tf.variable_scope("task_emb", reuse = tf.AUTO_REUSE):
        task_emb_var = tf.get_variable(name='task_emb_var',
                                 shape=[args.task_num, args.expert_layers[-1]],
                                 regularizer = tf.keras.regularizers.l2(args.lamda),
                                 initializer=tf.random_normal_initializer(mean=0.0, stddev=0.01))
        task_idx = [e for e in range(args.task_num)]
        task_id_emb = tf.nn.embedding_lookup(task_emb_var, task_idx)
        task_id_emb = tf.expand_dims(task_id_emb, axis=0)
    ...
    task_tower_inputs = task_tower_inputs + task_id_emb
